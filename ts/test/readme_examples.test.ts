...
//     (\`new PhantomjscloudSDK(...)\` / \`PhantomjscloudSDK.test(...)\` become
//     \`PhantomjscloudSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PhantomjscloudSDK'
...
const TEST_SEED = {"entity":{"render_page_get":{"test01":{"id":"test01"}},"render_page_post":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PhantomjscloudSDK(...)\` and any
// \`PhantomjscloudSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PhantomjscloudSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PhantomjscloudSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
