...
describe('RenderPagePostEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'render_page_post.' + op, live)) return
    }
    ...
    render_page_post_ref01_data = await render_page_post_ref01_ent.create(render_page_post_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['PHANTOMJSCLOUD_TEST_RENDER_PAGE_POST_ENTID']
