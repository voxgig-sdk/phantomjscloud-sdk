...
import (
	...
	omni "github.com/voxgig-sdk/phantomjscloud-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PhantomjscloudError); is {
				out := map[string]any{
					"name":    "PhantomjscloudError",
...
func omniCtx(arg any, client *sdk.PhantomjscloudSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PhantomjscloudSDK); is {
			client = live
