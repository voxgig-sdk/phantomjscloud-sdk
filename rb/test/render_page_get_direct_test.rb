...
class RenderPageGetDirectTest < Minitest::Test
  def test_direct_load_render_page_get
    setup = render_page_get_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-render_page_get", setup[:live] ? "live" : "unit")
    if _should_skip
...
def render_page_get_direct_setup(mockres)
  ...
  if live
    ...
    client = PhantomjscloudSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
