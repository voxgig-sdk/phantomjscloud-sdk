...
require_relative 'Phantomjscloud_types'
...
class PhantomjscloudSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PhantomjscloudHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PhantomjscloudHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PhantomjscloudFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PhantomjscloudUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PhantomjscloudHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PhantomjscloudHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PhantomjscloudHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PhantomjscloudError => err
    ...
    if fetched.is_a?(Hash)
      status = PhantomjscloudHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PhantomjscloudError.new(
        "graphql_error", "PhantomjscloudSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PhantomjscloudSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
