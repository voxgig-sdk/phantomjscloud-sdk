# Phantomjscloud Ruby SDK

The Ruby SDK for the Phantomjscloud API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.RenderPageGet(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Phantomjscloud(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── Phantomjscloud_sdk.rb       -- Main SDK module
