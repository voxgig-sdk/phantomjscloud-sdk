...
module PhantomjscloudUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PhantomjscloudResponse.new({})
    ...
    if fetch_err
      ...
      response = PhantomjscloudResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PhantomjscloudResponse.new(fetched)
    else
