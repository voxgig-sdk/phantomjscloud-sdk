...
class RenderPageGetEntity
  ...
  def make
    opts = @_entopts.dup
    RenderPageGetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PhantomjscloudHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RenderPageGet, Hash] the current RenderPageGet data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PhantomjscloudHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RenderPageGet fields)
  def match_get
  ...
  #
  # @param reqmatch [RenderPageGetLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RenderPageGet.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RenderPageGet, Hash] the loaded RenderPageGet; raises PhantomjscloudError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PhantomjscloudHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
