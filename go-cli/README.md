...
boru-driven command-line client **and** interactive REPL for the Phantomjscloud
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/phantomjscloud-cli)
make build

# 2. See usage (words, entities, env vars)
./phantomjscloud-cli --help

# 3. Provide credentials once, via the environment
export PHANTOMJSCLOUD_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./phantomjscloud-cli load 1 render_page_get            # {id:1} shorthand
./phantomjscloud-cli load '{id:1}' render_page_get       # explicit match map

# 5. Override the API base URL for a single call
PHANTOMJSCLOUD_BASE=https://api.example.com ./phantomjscloud-cli load 1 render_page_get

# 6. No arguments -> interactive REPL
./phantomjscloud-cli
phantomjscloud> load 1 render_page_get
phantomjscloud> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/phantomjscloud-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/phantomjscloud-cli load 1 render_page_get
   ```
...
```sh
./phantomjscloud-cli load 1 render_page_get          # scalar shorthand for {id:1}
./phantomjscloud-cli load '{id:1}' render_page_get     # explicit match map
```
...
./phantomjscloud-cli load 1 render_page_get
```
...
Run with no arguments to open a REPL (prompt `phantomjscloud>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `render_page_get`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/phantomjscloud-cli`. |
...
(`./phantomjscloud-cli <expr>`) and interactively (the REPL), and expressions compose the
