// Command Phantomjscloud-cli is a CLI + REPL for the Phantomjscloud SDK,
...
const prompt = "phantomjscloud"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "render_page_get render_page_post"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PHANTOMJSCLOUD_APIKEY (api key), PHANTOMJSCLOUD_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PHANTOMJSCLOUD_APIKEY carries the API key and
	// PHANTOMJSCLOUD_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PHANTOMJSCLOUD_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PHANTOMJSCLOUD_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPhantomjscloudSDK(opts)
