...
The Lua SDK for the Phantomjscloud API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:RenderPageGet(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Phantomjscloud(nil):load(
  { id = "test01" }, nil
)
