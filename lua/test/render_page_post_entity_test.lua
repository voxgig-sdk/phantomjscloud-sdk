...
describe("RenderPagePostEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:RenderPagePost(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = render_page_post_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "render_page_post." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PHANTOMJSCLOUD_TEST_RENDER_PAGE_POST_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local render_page_post_ref01_ent = client:RenderPagePost(nil)
    local render_page_post_ref01_data = helpers.to_map(vs.getprop(
...
function render_page_post_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/render_page_post/RenderPagePostTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read render_page_post test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("PHANTOMJSCLOUD_TEST_RENDER_PAGE_POST_ENTID")
