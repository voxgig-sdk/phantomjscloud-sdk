...
describe("RenderPageGetEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:RenderPageGet(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = render_page_get_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "render_page_get." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PHANTOMJSCLOUD_TEST_RENDER_PAGE_GET_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local render_page_get_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.render_page_get")))
    local render_page_get_ref01_data = nil
    if #render_page_get_ref01_data_raw > 0 then
      render_page_get_ref01_data = helpers.to_map(render_page_get_ref01_data_raw[1][2])
    end
...
function render_page_get_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/render_page_get/RenderPageGetTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read render_page_get test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("PHANTOMJSCLOUD_TEST_RENDER_PAGE_GET_ENTID")
