...
local RenderPagePostEntity = {}
RenderPagePostEntity.__index = RenderPagePostEntity


function RenderPagePostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RenderPagePostEntity)
  self._name = "render_page_post"
  self._client = client
...
function RenderPagePostEntity:get_name()
  return self._name
end


function RenderPagePostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RenderPagePostEntity.new(self._client, opts)
end
...
function RenderPagePostEntity:mark_deleted()
  self._deleted = true
end


function RenderPagePostEntity:deleted()
  return true == self._deleted
end


function RenderPagePostEntity:data_set(args)
  if args ~= nil then
...
function RenderPagePostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RenderPagePostEntity:match_set(args)
  if args ~= nil then
...
function RenderPagePostEntity:match_get()
...
function RenderPagePostEntity:stream(action, args, callopts)
...
function RenderPagePostEntity:_run_op(ctx, post_done)
  local utility = self._utility
