...
[MCP](https://modelcontextprotocol.io) server exposing the Phantomjscloud SDK as
two agent tools — `phantomjscloud_list` and `phantomjscloud_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/phantomjscloud-mcp)
make build

# 2. Provide credentials via the environment
export PHANTOMJSCLOUD_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user phantomjscloud \
  -- /absolute/path/to/phantomjscloud-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./phantomjscloud-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// phantomjscloud_list: first page of records
{ "entity": "render_page_get" }
{ "entity": "render_page_get", "query": { } }

// phantomjscloud_load: one record by id
{ "entity": "render_page_get", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/phantomjscloud-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user phantomjscloud \
     -- "$PWD"/dist/*/phantomjscloud-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `phantomjscloud_list` and `phantomjscloud_load` tools now appear
   in new sessions. Ask the agent to *"list render_page_get using phantomjscloud"*
   and it calls `phantomjscloud_list` with `{"entity":"render_page_get"}`.
...
```sh
export PHANTOMJSCLOUD_APIKEY=sk_live_xxx            # API key
export PHANTOMJSCLOUD_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `phantomjscloud_list` tool
...
### Call the `phantomjscloud_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `phantomjscloud_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `phantomjscloud_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PHANTOMJSCLOUD_APIKEY` | API key sent with every request. |
| `PHANTOMJSCLOUD_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

render_page_get | render_page_post

### Smoke test via HTTP (raw JSON-RPC)

```sh
./phantomjscloud-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"phantomjscloud_load","arguments":{"entity":"render_page_get","query":{"id":1}}}}'
```
