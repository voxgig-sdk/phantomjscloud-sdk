...
class TestRenderPagePostEntity:

    def test_should_create_instance(self):
        testsdk = PhantomjscloudSDK.test(None, None)
        ent = testsdk.RenderPagePost(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _render_page_post_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "render_page_post." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PHANTOMJSCLOUD_TEST_RENDER_PAGE_POST_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        render_page_post_ref01_ent = client.RenderPagePost(None)
        ...
        render_page_post_ref01_data = helpers.to_map(render_page_post_ref01_ent.create(render_page_post_ref01_data, None))
        assert render_page_post_ref01_data is not None



def _render_page_post_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/render_page_post/RenderPagePostTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "PHANTOMJSCLOUD_TEST_RENDER_PAGE_POST_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PHANTOMJSCLOUD_TEST_RENDER_PAGE_POST_ENTID"))
