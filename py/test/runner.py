# Phantomjscloud SDK test runner
...
class PhantomjscloudTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PhantomjscloudTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PhantomjscloudTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PhantomjscloudTestRunner.getenv("PHANTOMJSCLOUD_TEST_LIVE")
        override = PhantomjscloudTestRunner.getenv("PHANTOMJSCLOUD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PhantomjscloudTestRunner.getenv(key)
        ...
        explain = PhantomjscloudTestRunner.getenv("PHANTOMJSCLOUD_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PHANTOMJSCLOUD_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PhantomjscloudTestRunner._test_control is not None:
            return PhantomjscloudTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PhantomjscloudTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PhantomjscloudTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PhantomjscloudTestRunner.load_test_control()
...
def load_env_local():
    PhantomjscloudTestRunner.load_env_local()


def env_override(m):
    return PhantomjscloudTestRunner.env_override(m)


def entity_data(v):
    return PhantomjscloudTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PhantomjscloudTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PhantomjscloudTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PhantomjscloudTestRunner.load_test_control()


def live_client_options():
    return PhantomjscloudTestRunner.live_client_options()


def live_delay_ms():
    return PhantomjscloudTestRunner.live_delay_ms()
