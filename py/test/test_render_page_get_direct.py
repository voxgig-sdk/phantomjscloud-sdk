...
class TestRenderPageGetDirect:

    def test_should_direct_load_render_page_get(self):
        setup = _render_page_get_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-render_page_get", "live" if setup["live"] else "unit")
        if _skip:
...
def _render_page_get_direct_setup(mockres):
