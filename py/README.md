...
The Python SDK for the Phantomjscloud API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.RenderPageGet(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Phantomjscloud(None).load(
    {"id": "test01"}, None
)
