...
//    only - a typed PhantomjscloudContext there would make every ctx
...
final class PhantomjscloudOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PhantomjscloudContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new PhantomjscloudSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new PhantomjscloudResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new PhantomjscloudError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new PhantomjscloudResponse($clean['response']);
    ...
    public static function err_from_map($m): ?PhantomjscloudError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PhantomjscloudError(is_string($code) ? $code : '', $msg);
    }
