...
// Phantomjscloud SDK feature test
//
...
class FtCtrl extends PhantomjscloudControl
{
...
class FtClient
{
    ...
    private PhantomjscloudUtility $utility;

    public function __construct(PhantomjscloudUtility $utility, array $options)
    {
    ...
    public function get_utility(): PhantomjscloudUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PhantomjscloudContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PhantomjscloudContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PhantomjscloudContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PhantomjscloudConfig::make_feature($name);
    ...
    private static function buildUrl(PhantomjscloudSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PhantomjscloudError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PhantomjscloudResponse($fetched);
        }
    ...
    private function fail(PhantomjscloudContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PhantomjscloudContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
