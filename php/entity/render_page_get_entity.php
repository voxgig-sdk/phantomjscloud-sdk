...
// Phantomjscloud SDK RenderPageGet entity
...
class RenderPageGetEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RenderPageGetEntity($this->_client, $opts);
    }

    /**
     * @param RenderPageGet|array $args RenderPageGet data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PhantomjscloudHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RenderPageGet|array The current RenderPageGet data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RenderPageGet fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PhantomjscloudHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RenderPageGet fields).
     */
    ...
    /**
     * Load a single RenderPageGet.
     *
     * @param RenderPageGetLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RenderPageGetLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RenderPageGet|array The loaded RenderPageGet as an assoc-array at the
     *   SDK boundary; throws PhantomjscloudError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PhantomjscloudHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
