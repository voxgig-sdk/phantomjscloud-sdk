# Phantomjscloud PHP SDK Reference

Complete API reference for the Phantomjscloud PHP SDK.
...
$client = new PhantomjscloudSDK($options);
```
...
#### `PhantomjscloudSDK::test($testopts = null, $sdkopts = null)`
...
#### `RenderPageGet($data = null)`

Create a new `RenderPageGetEntity` instance. Pass `null` for no initial data.

#### `RenderPagePost($data = null)`
...
```php
[$result, $err] = $client->RenderPageGet()->load(["id" => "render_page_get_id"]);
```
...
```php
[$result, $err] = $client->RenderPagePost()->create([
  "url" => /* `$STRING` */,
]);
```
