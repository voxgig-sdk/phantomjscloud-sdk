...
// Phantomjscloud SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PhantomjscloudTestFeature extends PhantomjscloudBaseFeature
{
    ...
    public function init(PhantomjscloudContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PhantomjscloudContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PhantomjscloudParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PhantomjscloudContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PhantomjscloudContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PhantomjscloudParam::call($ctx, $k);
