...
// Phantomjscloud SDK retry feature
...
class PhantomjscloudRetryFeature extends PhantomjscloudBaseFeature
{
    ...
    public function init(PhantomjscloudContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PhantomjscloudContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PhantomjscloudContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PhantomjscloudContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
